...
switch($act){
    ...
    case 'admininfo':
        ...
        if (conf('Vaptcha_Open') == 1 && $token=='') {
            ...
            /*if ($adminPwd == $adminData['adminPwd']) {
                exit('{"code":0,"msg":"修改失败，未知错误。"}');
            }*/
            ...
            if(!$admininfo)exit('{"code":0,"msg":"未修改数据，无需保存！"}');
    ...
    case 'setProject':
        ...
        if ($type == 'Status') {
            ...
            $type=$_POST['type'];
            $paycontact=$_POST['paycontact'];
            $is_show=$_POST['is_show'];
            $Audit_status=$_POST['Audit_status'];
            $status=$_POST['status'];
            if($adminData['adminRank']==2){
                if ($name==NULL || $url==NULL || $img==NULL || $img2==NULL || $img3==NULL || $sketch==NULL || $descriptison==NULL || $money==NULL || $version==NULL || $type==NULL || $paycontact==NULL || $is_show==NULL || $status==NULL) {
                    exit('{"code":-1,"msg":"保存错误,请确保每项都不为空!"}');
                }
                $sds=$DB->exec("INSERT INTO `nteam_project_list` (`name`, `url`, `img`, `img2`, `img3`, `sketch`, `descriptison`, `money`, `version`, `type`, `paycontact`, `intime`, `status`, `Audit_status`, `is_show`) VALUES ('{$name}', '{$url}', '{$img}', '{$img2}', '{$img3}', '{$sketch}', '{$descriptison}', '{$money}', '{$version}', '{$type}', '{$paycontact}', '{$date}', '{$status}', '0', '{$is_show}')");
                $id=$DB->lastInsertId();
                if($sds){
                ...
                if ($name==NULL || $url==NULL || $img==NULL || $img2==NULL || $img3==NULL || $sketch==NULL || $descriptison==NULL || $money==NULL || $version==NULL || $type==NULL || $paycontact==NULL || $is_show==NULL || $Audit_status==NULL || $status==NULL) {
                    exit('{"code":-1,"msg":"保存错误,请确保每项都不为空!"}');
                }
                $sds=$DB->exec("INSERT INTO `nteam_project_list` (`name`, `url`, `img`, `img2`, `img3`, `sketch`, `descriptison`, `money`, `version`, `type`, `paycontact`, `intime`, `status`, `Audit_status`, `is_show`) VALUES ('{$name}', '{$url}', '{$img}', '{$img2}', '{$img3}', '{$sketch}', '{$descriptison}', '{$money}', '{$version}', '{$type}', '{$paycontact}', '{$date}', '{$status}', '{$Audit_status}', '{$is_show}')");
                $id=$DB->lastInsertId();
                if($sds){
            ...
            if($adminData['adminRank']==2){
                ...
                $sql="update `nteam_project_list` set `name` ='{$name}',`url` ='{$url}',`img` ='{$img}',`img2` ='{$img2}',`img3` ='{$img3}',`sketch` ='{$sketch}',`descriptison` ='{$descriptison}',`money` ='{$money}',`version` ='{$version}',`type` ='{$type}',`paycontact` ='{$paycontact}',`status` ='{$status}',`is_show` ='{$is_show}' where `id`='$id'";
                ...
                $sql="update `nteam_project_list` set `name` ='{$name}',`url` ='{$url}',`img` ='{$img}',`img2` ='{$img2}',`img3` ='{$img3}',`sketch` ='{$sketch}',`descriptison` ='{$descriptison}',`money` ='{$money}',`version` ='{$version}',`type` ='{$type}',`paycontact` ='{$paycontact}',`status` ='{$status}',`Audit_status` ='{$Audit_status}',`is_show` ='{$is_show}' where `id`='$id'";
                if($DB->exec($sql)!==false||$sqs){
                    $DB->query("insert into `nteam_log` (`adminUser`,`type`,`data`,`ip`,`city`) values ('" . $_SESSION['adminUser'] . "','修改了ID为".$id."的项目', NOW(), '".$ip."', '".$city."')");
                    exit('{"code":0,"msg":"修改项目信息成功！"}');
                }else{
    ...
    case 'setMember':
        ...
        if ($type == 'Status') {
            ...
            $is_show=$_POST['is_show'];
            $Audit_status=$_POST['Audit_status'];
            if($name==NULL || $qq==NULL || $describe==NULL || $teamimg==NULL || $is_show==NULL || $Audit_status==NULL){
                exit('{"code":-1,"msg":"保存错误,请确保每项都不为空!"}');
            } else {
                $sds=$DB->query("INSERT INTO `nteam_team_member` (`name`, `qq`, `describe`, `teamimg`, `is_show`, `Audit_status`, `intime`) VALUES ('{$name}', '{$qq}', '{$describe}', '{$teamimg}', '{$is_show}', '{$Audit_status}', NOW())");
                $id=$DB->lastInsertId();
                if($sds){
            ...
            if($name==NULL || $qq==NULL || $describe==NULL || $teamimg==NULL || $is_show==NULL || $Audit_status==NULL){
                ...
                $sql="update `nteam_team_member` set `name` ='{$name}',`qq` ='{$qq}',`describe` ='{$describe}',`teamimg` ='{$teamimg}',`is_show` ='{$is_show}',`Audit_status` ='{$Audit_status}' where `id`='$id'";
                if($DB->exec($sql)!==false||$sqs){
    ...
    case 'adminfk':
        ...
        $yun_url= "https://www.uaovo.com/Ajax.php?act=admintg&sub=".$sub."&msg=".$msg."&qq=".$qq."&token=".$token;
